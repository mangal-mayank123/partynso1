...
class RatingBarWidget extends State<RatingBar> {
  ...
  @override
  void initState() {
    y = double.parse(Post.rl[widget.x.data['user_id']][3]);
        ...
        child: Column(
          children: <Widget>[
            ...
            disable == false
                ? FormField(
                    onSaved: (v) {
                      Post.rl[widget.x.data['user_id']][3] =
                          (double.parse(Post.rl[widget.x.data['user_id']][3]) +
                                  rating)
                              .floor()
                              .toString();
                    },
              ...
              onPressed: disable == false
                  ? () {
                      setState(() {
                        disable = true;
                        rating = 0.0;
                        Post.rl[widget.x.data['user_id']][2] = (double.parse(
                                    Post.rl[widget.x.data['user_id']][2]) -
                                1)
                            .floor()
                            .toString();
                      });
